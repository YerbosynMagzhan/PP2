...
dis = pygame.display.set_mode((dis_wdth, dis_hght))
...
def message(msg, color):
    ...
    dis.blit(mes, [dis_wdth / 6, dis_hght / 3])
...
def gameLoop():
    ...
    foodx = round(random.randrange(0, dis_wdth - snake_block) / 10) * 10
    foody = round(random.randrange(0, dis_hght - snake_block) / 10) * 10
    foodn = round(random.randrange(0, dis_wdth - 15) / 10) * 10
    foodm = round(random.randrange(0, dis_hght - 15) / 10) * 10
    ...
    while not game_over:
        ...
        if x1 >= dis_wdth or x1 < 0 or y1 >= dis_hght or y1 < 0:
            game_close = True
        dis.fill(blue)
        #draw the food
        ...
        if x1 == foodn and y1 == foodm:
            foodn = round(random.randrange(0, dis_wdth - snake_block) / 10) * 10
            foodm = round(random.randrange(0, dis_hght - snake_block) / 10) * 10
            Length_of_snake += 2
